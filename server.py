...
APP_VERSION = "2026.09.16-ultra-realistic-prompt"   # bump mỗi lần đổi backend để check deploy
...
def lvt_quote_plan(quote, n, wordmark="", style_id="", ref_direction=""):
    ...
    sys = (LVT_BRAIN + style_lock + "\n\n"
           ...
           "SÁNG TẠO TỰ DO HẾT CỠ — không dập khuôn: typography (script tay / sans đậm khối / micro-text / "
           "mix size / cong / nghiêng…), minh hoạ (nhân vật tự nghĩ, con vật, đồ vật ẩn dụ, cảnh nhỏ, "
           "hoạ tiết, typo-only… — cấm nhại nhân vật bản quyền) và BẢNG MÀU đều tuỳ cảm hứng theo mood "
           "từng mẫu: đen trắng tối giản, pastel, neon, retro, rực rỡ nhiều màu… đều được. RÀNG BUỘC DUY "
           "NHẤT: các mẫu TRONG CÙNG LOẠT phải khác nhau rõ (kiểu chữ, vật thể minh hoạ, tông màu — đừng "
           "cả loạt cùng một vibe).\n"
...
def run_td_job(job_id, theme, text, sub, n, hint, use_art=True):  # use_art giữ cho tương thích, không dùng
    ...
    icons_desc = ", ".join(TD_ICONS)
    sys_p = TD_SYSTEM % (fonts_desc, icons_desc) + td_recipes_text()
    # Gợi ý đích danh font để đa dạng (AI hay bám anton/archivo)
    font_nudge = ""
    if not any(ord(c) > 127 for c in (text + " " + (sub or ""))):
        ...
        vn_pool = [k for k, v in TD_FONTS.items()
                   if "(EN-ONLY)" not in v[1] and k not in ("anton", "archivo", "bevietnam", "bevietnambold")]
        if vn_pool:
            picks = random.sample(vn_pool, min(max(n, 3), len(vn_pool)))
            font_nudge = ("\nĐA DẠNG FONT: mỗi bản dùng 1 font CHÍNH KHÁC NHAU — hãy dùng lần lượt: %s "
                          "(anton/archivo tối đa 1 bản)." % ", ".join(picks))
    ...
    def handle_fbpost_regen(self, body):
        ...
        try:
            ...
            if single or _FBP_ONE_SHIRT in pose:
                prompt += (" FINAL REMINDER: THIS shot shows exactly ONE shirt filling the frame — "
                           "the two-shirt rule applies to the whole SET, not to this close-up.")
            b64 = gen_shot_retry(imgs, prompt, size, engine, aspect, lock=False, quality=quality)
            if HAS_PIL:
                try:
                    b64 = base64.b64encode(crop_to_aspect(base64.b64decode(b64), aspect)).decode()
                except Exception:
    ...
    def handle_prod_generate(self, body):
        ...
        if (has_o1 or has_o2) and not has_pose:
            prompt += (" OUTFIT-SWAP MODE: keep each person's ORIGINAL pose, framing and background exactly as "
                       "in their own reference photo — but their shirt/garment is fully REPLACED by the clothing "
                       "reference. CRITICAL: do NOT carry over ANY print, text, logo or graphic from the clothes "
                       "they wear in their own photo — that old print must be GONE. If the clothing reference is "
                       "plain, the worn garment must be COMPLETELY BLANK fabric, front and back, no exceptions.")
