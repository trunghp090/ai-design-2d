...
let prodRoleImgs = { char: null, char2: null, bg: null, pose: null };   // ảnh nhân vật / bối cảnh / pose (tuỳ chọn)
...
function prodInit() {
  ...
  [["char", "prodCharFile"], ["char2", "prodChar2File"], ["bg", "prodBgFile"], ["pose", "prodPoseFile"]].forEach(([k, fid]) => {
...
const PROD_ROLE_UI = { char: ["prodCharRef", "prodCharFile", "👤", "Nhân vật 1"], char2: ["prodChar2Ref", "prodChar2File", "👥", "Nhân vật 2"], bg: ["prodBgRef", "prodBgFile", "🏞", "Bối cảnh"], pose: ["prodPoseRef", "prodPoseFile", "🧍", "Pose"] };
...
async function prodGenerate(prompt, count) {
  ...
  try {
    const r = await fetch("/api/prod-generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ images: prodRefs, style: prodStyle || "", char_img: prodRoleImgs.char || "", char2_img: prodRoleImgs.char2 || "", bg_img: prodRoleImgs.bg || "", pose_img: prodRoleImgs.pose || "", prompt, engine, aspect, count }) });
    const d = await r.json(); if (!r.ok) throw new Error(d.error || "Lỗi");
...
$("dsRunBtn").onclick = async () => {
  ...
  try {
    const r = await fetch("/api/design-gen", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ styles: [...dsPicked], ref: dsRefImg || "", auto_style: dsAuto, segment: dsSegment, theme: $("dsTheme").value, quote: ($("dsQuote")?.value || "").trim(), text: ($("dsText")?.value || ""), year: $("dsYear").value, same_line: ($("dsSameLine")?.checked || false), extra: ($("dsExtra")?.value || ""), n: parseInt($("dsCount").value, 10) || 3, size: $("dsSize").value, transparent: true }),
    });
    const d = await r.json();
    if (!r.ok) throw new Error(d.error || "Lỗi không xác định");
...
async function lvtPropose() {
  ...
  try {
    const r = await fetch("/api/lvt-propose", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ quote: quote, k: 8, illus_only: !$("lvtIllusOnly") || $("lvtIllusOnly").checked }) });
    const d = await r.json(); if (!r.ok) throw new Error(d.error || "Lỗi");
    lvtShowProps('🔎 Đề xuất cho quote “' + quote.replace(/</g, "&lt;") + '” — chọn mẫu ưng để gen theo', d.proposals || [], d.more || []);
    note.className = "gen-note ok"; note.textContent = "✓ " + (d.proposals || []).length + " đề xuất + " + (d.more || []).length + " ứng viên — xem HẾT ở cột phải.";
  } catch (e) { note.className = "gen-note err"; note.textContent = "✗ " + e.message; }
  btn.disabled = false; btn.textContent = o;
}
...
const PNL_METRICS = [
  ...
  { k: "atc",      ic: "🛒", label: "Thêm vào giỏ",      def: true },
  { k: "ic",       ic: "💳", label: "Tới checkout",      def: true },
  { k: "fb_purchase", ic: "✅", label: "Mua hàng (web)", def: true },
...
function pnlRenderCards(d) {
  ...
  const V = {
    ...
    atc:      [fmtNum(c.atc || 0), (d.funnel_src === "shopify" ? "web Shopify " : d.funnel_src === "fb" ? "từ FB Ads " : "") + pnlDelta(c.atc || 0, p.atc || 0, true)],
    ic:       [fmtNum(c.ic || 0), (c.atc ? "tỉ lệ " + Math.round((c.ic || 0) / c.atc * 100) + "% từ giỏ " : "") + pnlDelta(c.ic || 0, p.ic || 0, true)],
    fb_purchase: [fmtNum(c.fb_purchase || 0), (c.atc ? "chuyển đổi " + Math.round((c.fb_purchase || 0) / c.atc * 100) + "% từ giỏ " : "") + pnlDelta(c.fb_purchase || 0, p.fb_purchase || 0, true)],
