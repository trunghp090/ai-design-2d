(() => {
  ...
  root.innerHTML = `
    ...
    <div class="cc-grid">
      <section class="cc-panel"><h2><span class="cc-step">01</span>Thiết lập hội thoại</h2>
        ...
        <label class="cc-check"><input type="checkbox" id="cc-header" checked> Hiện thanh tên hội thoại</label>
  ...
  scriptPanel.querySelector('#cc-list').after(addButtons);
  ...
  function settings(){for(const k of ['name','ratio','font']) $(k).value=state[k];$('header').checked=state.header;}
  ...
  function list(){
    ...
    $('list').innerHTML=messages().map((m,i)=>`<article class="cc-message" data-message="${i}">
      ...
      <label class="cc-check"><input type="checkbox" data-field="heart" ${m.heart?'checked':''}>Hiện biểu tượng thả tim</label>
      ${m.kind==='image'?`<label class="cc-check"><input type="checkbox" data-field="hd" ${m.hd?'checked':''}>Hiện nhãn HD</label>`:''}
  ...
  async function draw(){
    ...
    root.querySelector('.cc-stage').style.setProperty('--cc-aspect',W/H);$('dimensions').textContent=`${W} × ${H}`;const canvas=$('canvas');canvas.width=W;canvas.height=H;canvas.getContext('2d').drawImage(buffer,0,0);
  }
  ...
  for(const k of ['name','ratio','font','header'])$(k).oninput=()=>{state[k]=k==='header'?$(k).checked:k==='font'?Math.max(24,Math.min(52,Number($(k).value)||36)):$(k).value;update();};
  ...
  $('list').oninput=e=>{
    ...
    }else{m[field]=e.target.type==='checkbox'?e.target.checked:e.target.value;draw().catch(e=>notice(e.message,true));persist();}
  };
...
window.zaloStudio = { async applyDraft(draft) {
    ...
    state=draft;slide=0;settings();resetEditor();update();return true;
  }};
})();
