...
function setup(confirmed=true){
 const els={}; const c={ttItems:[{gallery:{id:'ok'}},{gallery:{id:'fail'}}],ttConfirmDelete:async()=>confirmed,ttUpdateSel(){},ttRender(){},$:id=>els[id]||=( {} ),fetch:async url=>({ok:!url.includes('fail'),json:async()=>({ok:true})}),els};vm.createContext(c);vm.runInContext(code,c);return c;
}
